...
@Database(entities = {Stock.class, Customer.class, SaleDeatial.class, Sales.class, Expenses.class, ExpensesCategory.class, Payment.class, Purchase.class, ShopDetail.class}, version =71, exportSchema = false)
...
public abstract class SqlLiteDatabase extends RoomDatabase {
    ...
    public static synchronized SqlLiteDatabase getInstance(Context context) {
        if (instance == null) {
            instance = Room.databaseBuilder(context.getApplicationContext(), SqlLiteDatabase.class, "a2zBillDataBase71").allowMainThreadQueries().fallbackToDestructiveMigration().build();
        }
        return instance;
    }
